...
const BlogPage = () => {
	...
	const [hackerNewsStories, setHackerNewsStories] = useState<
		HackerNewsStory[]
	>([]);
	...
	const remainingArticles =
		devToArticles.length > 1 ? devToArticles.slice(1) : [];
						...
						<div className='lg:col-span-8 xl:col-span-9 space-y-8'>
									...
									<div className='aspect-video w-full overflow-hidden relative'>
										...
										<div className='absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex flex-col justify-end p-6 md:p-8'>
											<div className='flex flex-wrap gap-2 mb-3'>
												{featuredArticle.tag_list.map(
													(tag) => (
														<span
															key={tag}
															className='text-xs font-semibold px-2 py-1 rounded-md bg-primary text-primary-foreground backdrop-blur-sm'
														>
															#{tag}
														</span>
													)
							...
							<div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
								{remainingArticles.map((article) => (
									<div
										key={article.id}
										className='flex flex-col bg-card border border-border rounded-xl overflow-hidden hover:shadow-md transition-shadow group h-full'
									>
									...
									{hackerNewsStories.length > 0 ? (
										hackerNewsStories
											...
											.map((story) => (
												<div
													key={story.id}
													className='group relative pl-4 border-l-2 border-border hover:border-red-500 transition-colors duration-300'
												>
													<div
														onClick={() =>
															window.open(
